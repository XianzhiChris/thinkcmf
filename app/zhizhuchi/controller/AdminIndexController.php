<?php
namespace app\zhizhuchi\controller;

use cmf\controller\AdminBaseController;

use app\zhizhuchi\model\ZhizhuchiPostModel;
use app\zhizhuchi\service\PostService;
...
//            }
            ...
            $portalPostModel = new ZhizhuchiPostModel();
...
//            }
    ...
    {
        $param = $this->request->param();
        $where['renwu_id']=$param['id'];
//当前任务名称
        $PinglunQuery            = Db::name("zhizhuchi_post");
        $title=$PinglunQuery->field('post_title')->where('id',$param['id'])->find();
        $this->assign('title',$title['post_title']);

//        $postService = new PostService();
//        $data        = $postService->adminUrlList($param);
        $portalPostModel = Db::name('zhizhuchi_url');
